...
export default function EditForm({book}) {
    const { name: oldName, author: oldAuthor, thumbnail: oldThumbnail, price: oldPrice, rating: oldRating, featured: oldFeatured, id } = book;
    const navigate = useNavigate();
    const [editBook, {data: editedBook, isLoading}] = useEditBookMutation();
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        editBook({id, data: {name, author, thumbnail, price, rating, featured}});
    ...
    return (
        <form className="book-form" onSubmit={(e) => handleSubmit(e)}>
                    ...
                    <div className="space-y-2">
                        ...
                        <input required className="text-input" type="text" id="lws-thumbnail" name="thumbnail" value={thumbnail} onChange={(e) => setThumnail(e.target.value)} />
                    </div>

                    <div className="grid grid-cols-2 gap-8 pb-4">
                        <div className="space-y-2">
                            <label>Price</label>
                            <input required className="text-input" type="number" id="lws-price" name="price" value={price} onChange={(e) => setPrice(parseInt(e.target.value))} />
                        </div>
                    ...
                    <div className="flex items-center">
                        <input id="lws-featured" type="checkbox" name="featured" className="w-4 h-4" checked={featured} onClick={(e) => setFeatured(e.target.checked)} />
